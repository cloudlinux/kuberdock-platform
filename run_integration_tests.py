...
def run_tests_in_a_pipeline(pipeline_name, tests, seq_num, multilog,
    ...
    :param pipeline_name: name of a pipeline
    :param tests: list of callables - tests to run
    :param multilog: current multilogger instance
    :param cluster_debug: bool, cluster debug
    :return: None
    """
    ...
    print_msg("{} Cluster time: {}; Tests time: {};".format(
        pipeline_name, cluster_time, all_tests_time))
    print_msg("{} Detailed log: {}".format(
        pipeline_name, _get_thread_log(multilog, pipeline_name)))
...
def _get_thread_log(multilog, thread_name):
    """Get logs generated by a thread (pastebin link or raw)

    :param multilog: logger handler instance
    :param thread_name: name of a thread (usually name of a pipeline)
    """
    # Avoid handling/pasting same log twice
    if thread_name in thread_logs:
        return thread_logs[thread_name]

    thread_log = _format_log(
        thread_name,
        multilog.get_thread_log(thread_name)
    )
...
def _print_test_report(multilog, live_log):
    # If live log enabled all the logs were already printed.
    if not live_log:
        click.echo(center_text_message('PIPELINE DETAILED LOGS', fill_char='=',
                                       color=Fore.MAGENTA))
        # Iterating threads (not pipelines) here because there are extra ones.
        for t_name in multilog.thread_names:
            click.echo(
                '{}: {}'.format(t_name, _get_thread_log(multilog, t_name)),
                color=True
            )
