...
# Licensed under MIT.
# See https://github.com/fgimian/paramiko-expect/blob/master/LICENSE and
# paramiko_expect_LICENSE.txt copy
#
...
class SSHClientInteraction(object):
    ...
    def __init__(
        ...
        display=False, logger=None, encoding='utf-8'
    ):
    ...
    def expect(self, re_strings='', timeout=None):
        ...
        while (
            ...
            if self.display:
                self._log(current_buffer_decoded)
        ...
        while True:
            ...
            if current_line.endswith('\n'):
                if line_counter and callback:
                    self._log(callback(line_prefix, current_line))
                else:
                    if line_counter and line_prefix:
                        self._log(line_prefix)
                    if line_counter:
                        self._log(current_line)
                line_counter += 1
                current_line = ''
