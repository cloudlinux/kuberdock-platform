define(['pods_app/models/pods', 'sinon'], function(data, sinon){
    ...
    describe("Pod collection", function(){
        ...
        it("with method 'searchIn' can return models filtered by name", function(){
            podCollection.add([{name: 'one'},{name: 'two'},{name: 'three'},{name: 'thirteen'}])
            expect(podCollection.length).toBe(4);
            var filtered = podCollection.searchIn('th');
