...
class NebulaVMTemplate(object):
    ...
    def edit_disk(self, **kwargs):
        ...
        driver.text = kwargs.get('driver', 'raw')
        image = ET.SubElement(disk, 'IMAGE')
        image.text = kwargs.get('image')
        image_uname = ET.SubElement(disk, 'IMAGE_UNAME')
        image_uname.text = kwargs.get('image_uname')
        if kwargs.get('size'):
            size = ET.SubElement(disk, 'SIZE')
            size.text = kwargs.get('size')
    ...
    def wait_net_port(self, ip, port, timeout, try_interval=2):
        logging.info("Waiting for {0}:{1} to become available.".format(ip, port))
        end = time() + timeout
        while time() < end:
            try:
    ...
    def clone_template(self):
        ...
        # wait for image to be copied in datastore
    ...
    def run_template_updates(self):
        ...
        xml_to_update = self.edit_disk(
                        driver=template.xml.find(
                               'TEMPLATE/DISK/DRIVER').text,
                        image=image.name,
