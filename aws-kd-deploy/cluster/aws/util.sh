...
function deploy-nodes() {
    ...
    NODE_HOSTNAME=$(ssh -oStrictHostKeyChecking=no -i "${AWS_SSH_KEY}" "${SSH_USER}@${KUBE_NODE_IP_ADDRESSES[$i]}" hostname -f)
      ...
      echo "Adding node"
      if [[ ${TESTING} == "yes" ]]; then
      ssh -oStrictHostKeyChecking=no -i "${AWS_SSH_KEY}" -tt "${SSH_USER}@${KUBE_MASTER_IP}" "sudo python /var/opt/kuberdock/manage.py add_node --hostname=${NODE_HOSTNAME} --do-deploy --testing" < <(cat) 2>"$LOG"
      else
      ssh -oStrictHostKeyChecking=no -i "${AWS_SSH_KEY}" -tt "${SSH_USER}@${KUBE_MASTER_IP}" "sudo python /var/opt/kuberdock/manage.py add_node --hostname=${NODE_HOSTNAME} --do-deploy" < <(cat) 2>"$LOG"
      fi
    done
