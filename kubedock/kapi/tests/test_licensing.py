...
class TestLicenseValidation(unittest.TestCase):
    ...
    def test_license_is_valid_if_expired_less_than_3_days_ago(self):
        self.license[u'data'][u'license'][u'status'] = 'expired'
        self._move_license_expiration(-2)
        with mocked_license(self.license):
            self.assertTrue(licensing.is_valid())

    def test_license_is_valid_if_expired_exactly_3_days_ago(self):
        self.license[u'data'][u'license'][u'status'] = 'expired'
        self._move_license_expiration(-3)
        with mocked_license(self.license):
            self.assertTrue(licensing.is_valid())

    def test_license_is_not_valid_if_expired_more_than_3_days_ago(self):
        self.license[u'data'][u'license'][u'status'] = 'expired'
        self._move_license_expiration(-4)
        with mocked_license(self.license):
            self.assertFalse(licensing.is_valid())

    def test_new_but_incorrect_license_is_invalid(self):
