...
from kubedock.kapi import pod_states
from kubedock.pods import models as pod_models
from kubedock.users import models as user_models
...
class TestPodStates(DBTestCase):
    def setUp(self):
        super(TestPodStates, self).setUp()
    ...
    def test_save_state(self):
        ...
        host1 = 'host1'
        host2 = 'host2'
        pod_states.save_pod_state(pod1_id, 'ADDED', host1)
        ...
        pod_states.save_pod_state(pod1_id, 'MODIFIED', host1)
        pod_states.save_pod_state(pod2_id, 'ADDED', host2)
        states = PodState.query.all()
        ...
        pod_states.save_pod_state(pod1_id, 'ADDED', host2)
        ...
        pod_states.save_pod_state(pod1_id, 'DELETED', host2)
        states = PodState.query.order_by(PodState.start_time).all()
        self.assertEqual(len(states), 3)
        states = [item for item in states if item.pod_id == pod1_id]
        self.assertEqual(len(states), 2)
    ...
    def test_select_pod_states_history(self):
        ...
        host1 = 'host1'
        host2 = 'host2'
        res = pod_states.select_pod_states_history(pod1_id)
        self.assertEqual(res, [])

        pod_states.save_pod_state(pod1_id, 'ADDED', host1)
        pod_states.save_pod_state(pod1_id, 'MODIFIED', host1)
        pod_states.save_pod_state(pod1_id, 'DELETED', host1)
        pod_states.save_pod_state(pod1_id, 'ADDED', host1)
        res = pod_states.select_pod_states_history(pod1_id)
        ...
        last_state = res[0]
        res = pod_states.select_pod_states_history(pod1_id, 1)
        self.assertEqual(res, [last_state])

        # check filter by pod id is working
        pod_states.save_pod_state(pod2_id, 'ADDED', host1)
        res = pod_states.select_pod_states_history(pod1_id)
        self.assertEqual(len(res), 3)
